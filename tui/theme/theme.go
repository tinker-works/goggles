...
func New(args ...any) Theme {
	...
	isDark := true
	if len(args) > 0 {
		if palette, ok := args[0].(Palette); ok {
			p = palette
		}
		if paletteName, ok := args[0].(string); ok && paletteName != "" {
			name = paletteName
		}
	}
	if len(args) > 1 {
...
func (t Theme) WithBackground(isDark bool) Theme { return New(t.Palette, isDark) }
